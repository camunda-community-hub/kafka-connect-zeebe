...
public final class ZeebeSinkConnector extends SinkConnector {
  ...
  public ConfigDef config() {
    ...
    configDef.define(Constants.CONFIG_CORRELATION_KEY_JSONPATH, Type.STRING, "$.correlationKey", Importance.HIGH, "Variable name to gather correlation key to be used to correlate to a workflow instance from the message payload (e.g. correlationKey)");
    configDef.define(Constants.CONFIG_MESSAGE_NAME_JSONPATH, Type.STRING, "$.messageName", Importance.HIGH, "Variable name to gather message name to be send to Zeebe (e.g. messageName)");
  ...
  public List<Map<String, String>> taskConfigs(final int maxTasks) {
    ...
    for (int i = 0; i < maxTasks; i++) {
      ...
      config.put(Constants.CONFIG_CORRELATION_KEY_JSONPATH, correlationKeyVariable);
      config.put(Constants.CONFIG_MESSAGE_NAME_JSONPATH, messageNameVariable);
  ...
  public void start(final Map<String, String> props) {
    ...
    correlationKeyVariable = props.get(Constants.CONFIG_CORRELATION_KEY_JSONPATH);
    messageNameVariable = props.get(Constants.CONFIG_MESSAGE_NAME_JSONPATH);
